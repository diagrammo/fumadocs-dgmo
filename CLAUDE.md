...
- `remark-dgmo` and the `@diagrammo/dgmo` peer are open `>=X <1` ranges, in step with the other four wrappers; the dgmo floor never sits below the dgmo subpaths remark-dgmo imports. Other peers: `fumadocs-mdx` `^15`, `next` `^15 || ^16`, `react` `^19`
