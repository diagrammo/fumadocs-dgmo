...
📖 **Setup guide:** [diagrammo.app/embed#fumadocs](https://diagrammo.app/embed#fumadocs) · 🔭 **Live showcase:** [every chart type rendered through fumadocs-dgmo](https://diagrammo.github.io/fumadocs-dgmo/) — every block is in showcase mode, so hovering a diagram reveals its copy / open-in-editor footer.
...
```ts
// source.config.ts
mdxOptions: withDgmo({}, { dgmo: { liveLink: { refresh: 'render' } } }),
```

```tsx
// app/layout.tsx — beside <DgmoClient />
import { DgmoRenderClient } from 'fumadocs-dgmo/client-render';
```
